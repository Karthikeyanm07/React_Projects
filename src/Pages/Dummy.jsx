...
import { Clock, Trophy, RefreshCw, CheckCircle, XCircle } from "lucide-react";
...
const QuizApp = () => {
	...
	const [failedQuestions, setFailedQuestions] = useState([]); // ✅ Array, not object
	const [quizSet, setQuizSet] = useState(1); // ✅ Track which set (1 or 2)
	...
	const handleAnswerClick = (selectedOptionIndex) => {
		...
		if (selectedOptionIndex === currentQ.correctOption) {
			...
			// ✅ Store failed question correctly
			const failedQ = {
				qNo: currentQ.id,
	...
	return (
		<div className="bg-gradient-to-br from-purple-500 via-pink-500 to-red-500 min-h-screen w-full flex items-center justify-center p-4">
			<div className="bg-white w-full max-w-3xl shadow-2xl rounded-2xl overflow-hidden">
				{/* Quiz Set Selector */}
				{!showScore && (
					<div className="bg-gradient-to-r from-purple-600 to-pink-600 p-4 flex justify-center gap-4">
						<button
				...
				{showScore ? (
					...
					<div className="p-8">
						...
						<div className="flex gap-4 justify-center">
							<button
								className="flex items-center gap-2 text-white px-6 py-3 hover:bg-purple-700 transition-colors rounded-lg bg-purple-600 font-semibold"
								onClick={resetQuiz}
							>
							...
							<button
								className="flex items-center gap-2 text-white px-6 py-3 hover:bg-pink-700 transition-colors rounded-lg bg-pink-600 font-semibold"
								onClick={() =>
